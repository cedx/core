...
export class Error extends globalThis.Error
	...
	Object.defineProperty @::, "isClientError", get: -> 400 <= @status < 500

	# Value indicating whether the response's status code is between 500 and 599.
	Object.defineProperty @::, "isServerError", get: -> 500 <= @status < 600

	# The response's status code.
	Object.defineProperty @::, "status", get: -> @cause.status

	# The validation errors.
	Object.defineProperty @::, "validationErrors", get: ->
		if @_validationErrors? then Promise.resolve @_validationErrors else @_validationErrors = await @_parseValidationErrors()
