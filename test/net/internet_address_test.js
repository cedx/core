...
/**
 * Tests the features of the {@link InternetAddress} class.
 */
describe("InternetAddress", () => {
	...
	describe("isEmpty", () => {
		it("should return `true` if the address is empty", () => {
			assert.isTrue(new InternetAddress("").isEmpty);
			assert.isTrue(new InternetAddress(" \r\n ").isEmpty);
		});

		it("should return `false` if the address is not empty", () => {
			assert.isFalse(new InternetAddress("::1").isEmpty);
			assert.isFalse(new InternetAddress("192.168.100.1").isEmpty);
		});
	});

	describe("isValid", () => {
		it("should return `false` for an invalid address", () => {
			assert.isFalse(new InternetAddress("").isValid);
			assert.isFalse(new InternetAddress("159.180.abc.xyz").isValid);
			assert.isFalse(new InternetAddress("2z01:e0y:59m:1t20:5oo5:62g5:a8s8:1493").isValid);
		});

		it("should return `true` for a valid address", () => {
			assert.isTrue(new InternetAddress("::1").isValid);
			assert.isTrue(new InternetAddress("192.168.100.1").isValid);
			assert.isTrue(new InternetAddress("::ffff:192.168.100.1").isValid);
			assert.isTrue(new InternetAddress("2a01:e0a:59b:1f20:5dd5:62f5:a8a8:1493").isValid);
		});
	});
