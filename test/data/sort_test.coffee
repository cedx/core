...
describe "Sort", ->
	{deepEqual, equal, isAbove, isBelow, isEmpty, isFalse, isNull, isTrue, lengthOf} = assert
	...
	describe "[Symbol.iterator]()", ->
		it "should end iteration immediately if the sort is empty", ->
			iterator = new Sort()[Symbol.iterator]()
			isTrue iterator.next().done

		it "should iterate over the entries if the sort is not empty", ->
			iterator = Sort.of("foo").prepend("bar", SortOrder.desc)[Symbol.iterator]()
			next = iterator.next()
			isFalse next.done
	...
	describe "at()", ->
		...
		it "should return `null` for an unknown entry", -> isNull sort.at 1

	describe "compare()", ->
		x = index: 1, name: "abc", type: "object"
		y = index: 2, name: "xyz", type: "object"
	...
	describe "delete()", ->
		it "should properly remove entries", ->
			sort = new Sort [["foo", SortOrder.asc], ["bar", SortOrder.desc]]
			sort.delete "foo"
	...
	describe "get()", ->
		...
		it "should return `null` for an unknown entry", -> isNull sort.get "bar"
	...
	describe "has()", ->
		sort = Sort.of "foo"
		it "should return `true` an existing entry", -> isTrue sort.has "foo"
		it "should return `false` for an unknown entry", -> isFalse sort.has "bar"
	...
	describe "indexOf()", ->
		...
		it "should return `-1` for an unknown entry",
			-> equal sort.indexOf("qux"), -1
	...
	describe "satisfies()", ->
		...
		it "should return `true` if there is nothing to satisfy", ->
			isTrue sort.satisfies {}
			isTrue new Sort().satisfies attributes: ["foo"]

		it "should return `true` if the conditions are satisfied", ->
			isTrue sort.satisfies attributes: ["bar", "foo"], min: 1, max: 2

		it "should return `false` if the conditions are not satisfied", ->
			isFalse sort.satisfies attributes: ["baz"]
			isFalse sort.satisfies max: 1
	...
	describe "toSql()", ->
		...
		it "should allow custom escaping of attributes", ->
			sort = new Sort [["foo", SortOrder.asc], ["bar", SortOrder.desc]]
			equal sort.toSql((value) -> "[#{value}]"), "[foo] ASC, [bar] DESC"
